...
#    @fotos = Foto.all
  ...
  def create
    ...
    respond_to do |format|
      if @foto.save
        format.html { redirect_to index_fotos_path(:id => params[:foto][:proyecto_id]) , notice: 'Foto was successfully created.' }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to index_fotos_path(:id => id) }
      format.json { head :no_content }
    end
  end
end
